...
classdef LAKE_simple_salt_unfrozen_seb < SEB & HEAT_CONDUCTION & SALT
    ...
    methods
        ...
        function ground = provide_STATVAR(ground)
            ...
            ground.STATVAR.T_f = []; %temperature at which cell starts freezing
        ...
        function ground = merge_STATVAR(ground)  %merges all cell in one 
            ...
            ground.STATVAR.saltConc = sum(ground.STATVAR.saltConc,1);
        ...
        function ground = check_trigger(ground, tile)
            ...
            if ground.STATVAR.energy < ground.STATVAR.E_f  %freezing has started
        ...
        function ground = lateral_push_lake_water_exchange(ground, lateral)
            ...
            volume = ground.STATVAR.area .* ground.STATVAR.layerThick;
            fraction_exchanged = lateral.PARENT.IA_TIME_INCREMENT .* lateral.PARA.exchange_fraction_per_day;
            ground.STATVAR.saltConc = (1-fraction_exchanged) .* ground.STATVAR.saltConc + fraction_exchanged .* volume .* lateral.PARA.reservoir_saltConc;
            
        end
        
        function ground = lateral_push_lake_adjust_water_depth(ground, lateral)

            fraction = max(0, min(1, (lateral.PARENT.IA_TIME - lateral.PARA.start_date) ./ (lateral.PARA.end_date - lateral.PARA.start_date)));
            target_depth = lateral.PARA.start_depth + fraction .* (lateral.PARA.end_depth - lateral.PARA.start_depth);

            change_fraction = target_depth ./ ground.STATVAR.layerThick;

            ground.STATVAR.layerThick = ground.STATVAR.layerThick .* change_fraction;
            ground.STATVAR.waterIce = ground.STATVAR.waterIce .* change_fraction;  
            ground.STATVAR.mineral = ground.STATVAR.mineral .* change_fraction;
            ground.STATVAR.organic = ground.STATVAR.organic .* change_fraction; 
            ground.STATVAR.energy = ground.STATVAR.energy .* change_fraction; 
            ground.STATVAR.saltConc = ground.STATVAR.saltConc .* change_fraction; 
        end
