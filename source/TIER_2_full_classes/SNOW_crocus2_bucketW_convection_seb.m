%========================================================================
% CryoGrid GROUND class SNOW_crocus2_bucketW_convection_seb
...
classdef SNOW_crocus2_bucketW_convection_seb < SEB & HEAT_CONDUCTION & WATER_FLUXES & AIR_CONVECTION & HEAT_FLUXES_LATERAL & WATER_FLUXES_LATERAL & SNOW & SNOW_FLUXES_LATERAL & REGRID
    ...
    methods
        ...
        function snow = provide_PARA(snow)
            ...
            %convection
            snow.PARA.CozenyKarman_constant = 150;
            snow.PARA.Forchheimer_constant = 1.75;
        %    snow.PARA.thermal_expansion_coefficient_air = 0.0034; set to 1/T to get T-dependence
        ...
        function snow = finalize_init(snow, tile)
            ...
            snow.TEMP.d_water_vapour = snow.STATVAR.energy .*0;
        ...
        function snow = get_derivatives_prognostic(snow, tile)
            if size(snow.STATVAR.layerThick,1) > 1
                ...
                snow = get_derivative_air_convection_DarcyForchheimer(snow, tile);
        ...
        function snow = advance_prognostic(snow, tile)
            ...
            %mass
            snow.STATVAR.waterIce = snow.STATVAR.waterIce + timestep .* (snow.TEMP.d_water + snow.TEMP.d_water_vapour);
            snow.STATVAR.waterIce(1) = snow.STATVAR.waterIce(1) + timestep .* snow.STATVAR.sublimation;
            
            %determine whether sublimation is taken from within 1st cell or
            %from top of 1st cell
            % minus sublimation > d_water_vapour(1) -> do not add ice to matrix (=no densification), but subtract minus sublimation-d_water_vapour(1) from layerThick
            % minus sublimation < d_water_vapour(1) -> do not reduce layerThick, but add d_water_vapour(1) - minus sublimation to ice in the matrix (=dennsification)

            if snow.STATVAR.sublimation < 0
                if -snow.STATVAR.sublimation > snow.TEMP.d_water_vapour(1,1)
                    surface_sublimation = snow.STATVAR.sublimation + snow.TEMP.d_water_vapour(1,1); %negative when snow is subtracted, same as sublimation
                    volume_resublimation = 0;
                else
                    surface_sublimation = 0;
                    volume_resublimation = snow.STATVAR.sublimation + snow.TEMP.d_water_vapour(1,1); %positive when snow is added, same as d_water_vapor
                end
            else
                surface_sublimation = snow.STATVAR.sublimation;
                volume_resublimation = snow.TEMP.d_water_vapour(1,1);
            end

            %add internal sublimation/resublimation so that it can be taken into account in density calculation
            snow.STATVAR.ice(2:end,1) = snow.STATVAR.ice(2:end,1) + timestep .* snow.TEMP.d_water_vapour(2:end,1);
            snow.STATVAR.ice(1,1) = snow.STATVAR.ice(1,1) + timestep .* volume_resublimation;

            density_ice_phase = snow.STATVAR.ice(1) ./ snow.STATVAR.layerThickSnowFirstCell ./snow.STATVAR.area(1);
            snow.STATVAR.layerThickSnowFirstCell = snow.STATVAR.layerThickSnowFirstCell + timestep .* surface_sublimation ./snow.STATVAR.area(1,1) ./ density_ice_phase;
            
            snow.STATVAR.ice(1) = snow.STATVAR.ice(1) + timestep .* surface_sublimation;
            ...
            %CHANGED SW Dec 2025, prevent gs to be stuck at low diameter from initially low value until d=0 
            %IMPORTANT: do this also for other crocus snow classes
            snow.STATVAR.gs = max(double(snow.STATVAR.d>0).*1e-4.*(snow.STATVAR.d+(1-snow.STATVAR.d).*(4-snow.STATVAR.s)), max(snow.STATVAR.gs, snow.STATVAR.gs + timestep .*(snow.TEMP.metam_d_gs + snow.TEMP.wind_d_gs)));
        ...
        function snow = compute_diagnostic(snow, tile)
            ...
            %convection;
            if size(snow.STATVAR.layerThick,1) > 1
                snow.STATVAR.effective_grain_size = double(snow.STATVAR.d>0).*1e-4.*(snow.STATVAR.d+(1-snow.STATVAR.d).*(4-snow.STATVAR.s)) + ...
                    double(snow.STATVAR.d==0).*(snow.STATVAR.gs.*snow.STATVAR.s+(1-snow.STATVAR.s).*max(snow.STATVAR.gs.*0 + 4e-4, 2.*snow.STATVAR.gs)); %different from opt. dimater, increase gs to simulate higher permeability for non-sperical snow = depth hoar
                snow = permeability_air_Carman_Kozeny2(snow);
                snow = Forchheimer_facor(snow);
                snow = calculate_air_density_viscosity_convection(snow, tile);
                snow = calculate_velocity_convection(snow);
            end
        ...
        function snow = compute_diagnostic_CHILD(snow, tile)
            ...
            % snow.STATVAR.effective_grain_size = double(snow.STATVAR.d>0).*1e-4.*(snow.STATVAR.d+(1-snow.STATVAR.d).*(4-snow.STATVAR.s)) + ...
            %     double(snow.STATVAR.d==0).*(snow.STATVAR.gs.*snow.STATVAR.s+(1-snow.STATVAR.s).*max(snow.STATVAR.gs.*0 + 4e-4, snow.STATVAR.gs/2));
