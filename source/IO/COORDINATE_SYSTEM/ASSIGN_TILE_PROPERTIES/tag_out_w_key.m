        ...
        function update = assign_tile_properties(update, run_number)
            for i=1:size(update.PARA.target_class_name,1)
                ...
                if update.PARA.append == 0 || isempty(str) || sum(isnan(str))>0
                    update.PROJ.RUN_INFO.PPROVIDER.CLASSES.(update.PARA.target_class_name{i,1}){update.PARA.target_class_index(i,1),1}.PARA.tag2 = num2str(update.PROJ.RUN_INFO.SPATIAL.STATVAR.key(run_number,1));
                else
                    update.PROJ.RUN_INFO.PPROVIDER.CLASSES.(update.PARA.target_class_name{i,1}){update.PARA.target_class_index(i,1),1}.PARA.tag2 = [str '_' num2str(update.PROJ.RUN_INFO.SPATIAL.STATVAR.key(run_number,1))];
                end
            end
        end
