%========================================================================
% CryoGrid TILE class TILE_1D
% TILE class designed for multi-physics simulations with a stratigraphy of
% connected CryoGrid stratigraphy classes. TILE_1D represents all 
...
% environments and between different TILE_1D classes in a parallel
% fashion
...
% S. Westermann, Dec 2020, 2025 - combines functionalities of
% TILE_1D_standard and TILE_1D_standard2, also compatible with new
% optimization framework 
%========================================================================

classdef TILE_1D < matlab.mixin.Copyable
    ...
    methods
        ...
        function tile = store_OUT_tile_1(tile)
            tile.OUT = store_OUT(tile.OUT, tile);
        end        

        function tile = store_OUT_tile_2(tile)
        ...
        function tile = add_OUT_class(tile, out_class, out_class_index)
            if iscell(tile.PARA.out_class)
                ...
                tile.PARA.out_class = {tile.PARA.out_class};
                tile.PARA.out_class = [tile.PARA.out_class; out_class];
            end
            tile.PARA.out_class_index = [tile.PARA.out_class_index; out_class_index];
        end

        function  tile = add_OUT_class2(tile, out_class)
            if iscell(tile.OUT)
                tile.OUT = [tile.OUT; {out_class}];
            else
                tile.OUT = {tile.OUT};
                tile.OUT = [tile.OUT; {out_class}];
            end
        ...
        function tile = run_model(tile)
            if iscell(tile.OUT)
                tile = run_model_2(tile);
            else
                tile = run_model_1(tile);
            end
        end


        %OUT is a single class
        function tile = run_model_1(tile)
            ...
            %TIME INTEGRATION
            %=========================================================================
            while tile.t < tile.PARA.end_time
        ...
        %out is a cell array
        function tile = run_model_2(tile) 
        ...
        function tile = build_tile_new_init(tile)
            ...
            tile.PARA.end_time = tile.FORCING.PARA.end_time;
            tile.PARA.start_time = tile.FORCING.PARA.start_time;
...
%             end
        ...
        function tile = build_tile_restart_OUT_last_timestep(tile)
            ...
            if ~isempty(new_end_time)
                ...
                tile.PARA.end_time = new_end_time;
            end
