...
classdef TILE_ML < matlab.mixin.Copyable
    ...
    methods
        ...
        function tile = finalize_init(tile)
            ...
            tile.TEMP.target_variables = target_data_class.PARA.variables;
            ...
            tile.TARGET = target_data_class;
            tile.FEATURES = feature_data_class;
            ...
            for i=1:size(tile.TEMP.target_variables,1)
                tile.TEMP.var_ID = i;
                ...
                tile.OUT.ML{i,1} = tile.ML;
            end
            tile.STATVAR.out = target_ML;

            tile = assign_timestamp_prediction(feature_data_class, tile);
            %loop 10 times so that std can be computed
        end

        function tile = run_model(tile)
            for var_ID = 1:size(tile.TEMP.target_variables,1)
                tile.ML = tile.OUT.ML{var_ID,1};
                out_all=[];
                for i = 1:size(tile.STATVAR.timestamp_prediction,1)
                    tile.t = tile.STATVAR.timestamp_prediction(i,1);

                    in = generate_feature_data_prediction_single_timestamp(tile.FEATURES, tile);
                    in = (in - tile.STATVAR.in_mean) ./ tile.STATVAR.in_std;
                    [out, ~] = progapagate_ML(tile.ML, in);
                    out = mean(out,2) .* tile.STATVAR.out_std(1,var_ID) + tile.STATVAR.out_mean(1,var_ID);
                    out_all = [out_all out];
                end
                tile.OUT.(tile.TEMP.target_variables{var_ID,1}) = out_all;
            end
        end
