...
classdef process_topoScale_NORA3 < process_BASE
    ...
    methods
        ...
        function forcing = process(proc, forcing, tile)
            ...
            era_alt=double(era.Z(ind_lon, ind_lat, :, :));
            era_alt=reshape(era_alt, 4, size(era.Z,3), size(era.Z,4));
            
            era_alt_sl  = double(era.Zs(ind_lon, ind_lat));
            era_alt_sl = reshape(era_alt_sl, 4,1);
            era_alt_sl = repmat(era_alt_sl,1,1,size(era_alt,3));
            
            era_T=double(era.T(ind_lon, ind_lat, :, :));
            era_T=reshape(era_T, 4, size(era.T,3), size(era.T,4)).*era.T_sf;
            era_u=double(era.u(ind_lon, ind_lat, :, :)).* era.wind_sf;
            era_u=reshape(era_u, 4, size(era.u,3), size(era.u,4));
            era_v=double(era.v(ind_lon, ind_lat, :, :)).* era.wind_sf;
            era_v=reshape(era_v, 4, size(era.v,3), size(era.v,4));
            era_RH=era.RH_pl(ind_lon, ind_lat, :, :);
            era_RH=double(reshape(era_RH, 4, size(era.RH_pl,3), size(era.RH_pl,4))).*era.T_sf;
...
%             era_q=double(reshape(era_q, 4, size(era.q,3), size(era.q,4))).*era.q_sf;
            ...
            era_T_sl = reshape(double(era_T_sl, 4,1, size(era_T_sl,3))).*era.T_sf;
            era_RH_sl  = era.RH(ind_lon, ind_lat, :);
            era_RH_sl = reshape(double(era_RH_sl, 4,1, size(era_RH_sl,3))).*era.T_sf;
            era_wind_sl = sqrt(double(era.u10(ind_lon, ind_lat, :)).^2 + doubke(era.v10(ind_lon, ind_lat, :)) .^2).* era.wind_sf;
