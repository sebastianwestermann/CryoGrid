%========================================================================
% CryoGrid LATERAL_IA class LAT3D_WATER_UNCONFINED_AQUIFER_RICHARDS_EQ_OVERLAND_FLOW_XICE 
...
classdef LAT3D_WATER_UNCONFINED_AQUIFER_RICHARDS_EQ_OVERLAND_FLOW_XICE < BASE_LATERAL
    ...
    methods
        ...
        function lateral = pull(lateral, tile)
            ...
            lateral.PARENT.STATVAR.Xwater = [];
            lateral.PARENT.STATVAR.layerThick = [];            
        %    lateral.PARENT.STATVAR.ground_surface_elevation = []; %NEW SEBASTIAN for hillslope flow between grid cells
            ...
            while ~(strcmp(class(CURRENT), 'Bottom')) && lateral.TEMP.open_system == 1
                ...
                CURRENT = lateral3D_pull_Xwater_unconfined_aquifer(CURRENT, lateral); %to DO!!
            ...
            for j=1:size(lateral.PARENT.ENSEMBLE,1)
                if lateral.PARENT.PARA.connected(lateral.PARENT.STATVAR.index, lateral.PARENT.ENSEMBLE{j,1}.index)  %add if water is available at all
                    ...
                    for i=1:size(lateral.PARENT.ENSEMBLE{j,1}.overlap,1) %does not do anything when overlap is empty!
                        ...
                        %lateral fluxes due to "trapped" Xwater
                        flux_i_Xwater_in = double(lateral.PARENT.ENSEMBLE{j,1}.Xwater(lateral.PARENT.ENSEMBLE{j,1}.overlap(i,2),1) > 0 && lateral.PARENT.STATVAR.Xwater(lateral.PARENT.ENSEMBLE{j,1}.overlap(i,1),1) == -1) .* ...
                            lateral.PARENT.ENSEMBLE{j,1}.overlap(i,3) ./ lateral.PARENT.ENSEMBLE{j,1}.layerThick(lateral.PARENT.ENSEMBLE{j,1}.overlap(i,2),1) ./ (lateral.PARA.ia_time_increment .* lateral.PARENT.CONST.day_sec) .* ...
                            0.2 .* lateral.PARENT.ENSEMBLE{j,1}.Xwater(lateral.PARENT.ENSEMBLE{j,1}.overlap(i,2),1);
                        flux_i_Xwater_out = -double(lateral.PARENT.ENSEMBLE{j,1}.Xwater(lateral.PARENT.ENSEMBLE{j,1}.overlap(i,2),1) == -1 && lateral.PARENT.STATVAR.Xwater(lateral.PARENT.ENSEMBLE{j,1}.overlap(i,1),1) >0) .* ...
                            lateral.PARENT.ENSEMBLE{j,1}.overlap(i,3) ./ lateral.PARENT.STATVAR.layerThick(lateral.PARENT.ENSEMBLE{j,1}.overlap(i,1),1) ./ (lateral.PARA.ia_time_increment .* lateral.PARENT.CONST.day_sec) .* ...
                            0.2 .* lateral.PARENT.STATVAR.Xwater(lateral.PARENT.ENSEMBLE{j,1}.overlap(i,1),1);
                        
                        flux(lateral.PARENT.ENSEMBLE{j,1}.overlap(i,1),1) = flux(lateral.PARENT.ENSEMBLE{j,1}.overlap(i,1),1) + flux_i + flux_i_Xwater_in + flux_i_Xwater_out;
