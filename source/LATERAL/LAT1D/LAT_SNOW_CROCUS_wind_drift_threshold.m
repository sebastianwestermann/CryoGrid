...
classdef LAT_SNOW_CROCUS_wind_drift_threshold < BASE_LATERAL
    ...
    methods
        ...
        function lateral = provide_PARA(lateral)
            lateral.PARA.exposure_max_alt = []; %exposure positive = ablation; exposure negative = deposition 
            lateral.PARA.exposure_min_alt = [];
            lateral.PARA.max_altitude = [];
            lateral.PARA.min_altitude = [];            
        ...
        function lateral = finalize_init(lateral, tile)
            lateral.PARA.snow_holding_height = 0;
        end
        ...
        function lateral = push(lateral, tile)
            ...
            while ~(strcmp(class(CURRENT), 'Bottom'))
                if strcmp(class(CURRENT), 'SNOW_crocus_bucketW_seb') || strcmp(class(CURRENT), 'SNOW_crocus2_bucketW_seb')
                    CURRENT = lateral_pull_snow_wind_drift_threshold(CURRENT, lateral);
