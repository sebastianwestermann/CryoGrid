...
%             % Another Stability correction from CoupModel based on a Richards number:
%             % http://www.coupmodel.com/WordDocuments/aerodynamicresistancewithstabilitycorrectionbelowvegetationcanopy1.htm
%             if strcmp(tile.PARA.run_name,'Terelj_test')
%                 Ri = 9.81*(seb.STATVAR.Ts - get_surface_T(seb.NEXT, tile))*(z-d)./uz.^2./(seb.STATVAR.Ts + seb.CONST.Tmfw);
%                 f_Ri = double(Ri>0).*(1+16*Ri).^.33 + double(Ri<=0).*(1-16.*Ri).^-.33;
%                 seb.TEMP.r_a_prime = seb.TEMP.r_a_prime .* f_Ri;
%             end
