...
%                 ensemble.STATVAR.(ensemble.PARA.lognormal_variables_name{i,1}) = ensemble.STATVAR.(ensemble.PARA.lognormal_variables_name{i,1})(1,tile.PARA.worker_number);
        ...
        % 
        %             result.(variable_list{i,1}) = get_weighetd_average_of_parameter(ensemble, parameter, weights, clipping_threshold);
        %             %diag(parameter(:,pos));
        ...
        %             parameter = reshape(ensemble.TEMP.value_gaussian(pos_in_var_list,:), tile.PARA.number_of_realizations, size(ensemble.TEMP.value_gaussian,2) ./ tile.PARA.number_of_realizations);
        %             result.(variable_list{i,1}) = exp(get_weighetd_average_of_parameter(ensemble, parameter, weights, clipping_threshold)));
        %         end
        ...
        % end
        % 
        % function res = get_weighted_average_of_parameter(ensemble, parameter, weights, clipping_threshold)
        % 
