%========================================================================
% CryoGrid GROUND class GROUND_fcSimple_salt_buoyancy_seb_snow
...
classdef GROUND_fcSimple_salt_buoyancy_seb_snow < GROUND_fcSimple_salt_buoyancy_seb
    properties
        CHILD
        IA_CHILD
    end
    ...
    methods
        ...
        function ground = provide_PARA(ground)
            ground = provide_PARA@GROUND_fcSimple_salt_buoyancy_seb(ground);
        end
        
        function ground = provide_CONST(ground)
            ground = provide_CONST@GROUND_fcSimple_salt_buoyancy_seb(ground);
        end
        
        function ground = provide_STATVAR(ground)
            ground = provide_STATVAR@GROUND_fcSimple_salt_buoyancy_seb(ground);
        end
        
        function ground = finalize_init(ground, tile)
            ground = finalize_init@GROUND_fcSimple_salt_buoyancy_seb(ground, tile);
        ...
        function ground = get_boundary_condition_u(ground, tile)
            ...
            if ground.CHILD == 0  %SNOW CHILD does not exist
                ground = get_boundary_condition_u@GROUND_fcSimple_salt_buoyancy_seb(ground, tile); %call the native function for the ground class
...
%                     ground.CHILD = copy(CURRENT.STORE.SNOW);
                ...
                ground = get_boundary_condition_u@GROUND_fcSimple_salt_buoyancy_seb(ground, tile);
        ...
        function [ground, S_up] = penetrate_SW(ground, S_down)  %mandatory function when used with class that features SW penetration
            [ground, S_up] = penetrate_SW@GROUND_fcSimple_salt_buoyancy_seb(ground, S_down);
        end
        
        function ground = get_boundary_condition_l(ground, tile)
            %forcing = tile.FORCING;
            ground = get_boundary_condition_l@GROUND_fcSimple_salt_buoyancy_seb(ground, tile);
            
        end
        
        function ground = get_derivatives_prognostic(ground, tile)
            if ground.CHILD == 0
                ground = get_derivatives_prognostic@GROUND_fcSimple_salt_buoyancy_seb(ground, tile); %call normal function
            else
                ground.CHILD = get_derivatives_prognostic_CHILD(ground.CHILD, tile);
                ground = get_derivatives_prognostic@GROUND_fcSimple_salt_buoyancy_seb(ground, tile);
            end
        end
        
        function timestep = get_timestep(ground, tile)
            if ground.CHILD == 0
                timestep = get_timestep@GROUND_fcSimple_salt_buoyancy_seb(ground, tile);
            else
                timestep_snow = get_timestep_CHILD(ground.CHILD, tile);
                timestep_ground =  get_timestep@GROUND_fcSimple_salt_buoyancy_seb(ground, tile);
        ...
        function ground = advance_prognostic(ground, tile) 
            ...
            if ground.CHILD == 0
                ground =  advance_prognostic@GROUND_fcSimple_salt_buoyancy_seb(ground, tile);
            else
        ...
        function ground = compute_diagnostic(ground, tile)
            %forcing = tile.FORCING;
            if ground.CHILD == 0
                ground = compute_diagnostic@GROUND_fcSimple_salt_buoyancy_seb(ground, tile);
            else
                ground = compute_diagnostic@GROUND_fcSimple_salt_buoyancy_seb(ground, tile);
