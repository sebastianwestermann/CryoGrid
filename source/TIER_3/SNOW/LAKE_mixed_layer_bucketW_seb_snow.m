%========================================================================
% CryoGrid GROUND class LAKE_mixed_layer_bucketW_seb_snow
...
classdef LAKE_mixed_layer_bucketW_seb_snow < LAKE_mixed_layer_bucketW_seb
    properties
        CHILD
        IA_CHILD
    end
    ...
    methods
       ...
       function ground = provide_PARA(ground)  
            ground = provide_PARA@LAKE_mixed_layer_bucketW_seb(ground);
       end
       
       function ground = provide_CONST(ground)  
           ground = provide_CONST@LAKE_mixed_layer_bucketW_seb(ground);
       end
       
       function ground = provide_STATVAR(ground)  
           ground = provide_STATVAR@LAKE_mixed_layer_bucketW_seb(ground);
       end
       
       function ground = finalize_init(ground, tile)
           ground = finalize_init@LAKE_mixed_layer_bucketW_seb(ground, tile);
        ...
        function ground = get_boundary_condition_u(ground, tile)
            ...
            if ground.CHILD == 0  %CHILD does not exist
                ground = get_boundary_condition_u@LAKE_mixed_layer_bucketW_seb(ground, tile); %call the native function for the ground class
                
                if forcing.TEMP.snowfall > 0 && ground.STATVAR.energy(1,1) < 0   %create CHILD 
                ...
                ground = get_boundary_condition_u@LAKE_mixed_layer_bucketW_seb(ground, tile);
        ...
        function ground = get_boundary_condition_l(ground, tile)
              ground = get_boundary_condition_l@LAKE_mixed_layer_bucketW_seb(ground, tile);
        end
        
        function [ground, S_up] = penetrate_SW(ground, S_down)  %mandatory function when used with class that features SW penetration
            [ground, S_up] = penetrate_SW@LAKE_mixed_layer_bucketW_seb(ground, S_down);
        end
        
        function ground = get_derivatives_prognostic(ground, tile)
            if ground.CHILD == 0  
                ground = get_derivatives_prognostic@LAKE_mixed_layer_bucketW_seb(ground, tile); %call normal function
            else
                ground.CHILD = get_derivatives_prognostic_CHILD(ground.CHILD, tile);
                ground = get_derivatives_prognostic@LAKE_mixed_layer_bucketW_seb(ground, tile); 
            end
        end
        
        function timestep = get_timestep(ground, tile) 
            if ground.CHILD == 0
                timestep = get_timestep@LAKE_mixed_layer_bucketW_seb(ground, tile);
            else 
                timestep_snow = get_timestep_CHILD(ground.CHILD, tile);
                timestep_ground =  get_timestep@LAKE_mixed_layer_bucketW_seb(ground, tile);
        ...
        function ground = advance_prognostic(ground, tile) 
            if ground.CHILD == 0
                ground =  advance_prognostic@LAKE_mixed_layer_bucketW_seb(ground, tile);
            else                
        ...
        function ground = compute_diagnostic(ground, tile)
            if ground.CHILD == 0
                ground = compute_diagnostic@LAKE_mixed_layer_bucketW_seb(ground, tile);
            else
                ground = compute_diagnostic@LAKE_mixed_layer_bucketW_seb(ground, tile);
        ...
        function ground = check_trigger(ground, tile)
            ...
            if ground.CHILD ~= 0
                ...
                if ground.CHILD.STATVAR.area ./ ground.STATVAR.area(1,1) < 1e-6 %cutoff to get rid of remaining snow
                   ...
                   %merge CHILD with uppermost water cell
                elseif ground.STATVAR.energy(1,1) > 0
                    ground.STATVAR.energy(1,1) = ground.STATVAR.energy(1,1) + ground.CHILD.STATVAR.energy;
                    ground.STATVAR.waterIce(1,1) = ground.STATVAR.waterIce(1,1) + ground.CHILD.STATVAR.waterIce;
                    ground.STATVAR.layerThick(1,1) = ground.STATVAR.layerThick(1,1) + ground.CHILD.STATVAR.waterIce ./ ground.STATVAR.area(1,1);
                    ground.CHILD = 0;
                    ground.IA_CHILD = 0;
           ...
           %lake trigger 
           dummy = check_trigger@LAKE_mixed_layer_bucketW_seb(ground, tile);
        end
